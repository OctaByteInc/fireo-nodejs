...
class Manager {
  ...
  async save(merge = false) {
    ...
    await doc.set(this.meta.fieldMapping, { merge: merge });
    return doc.id;
  }
  ...
  async update(key) {
    let doc;
    if (key) {
      doc = Database.doc(key);
    } else {
      doc = Database.collection(this.meta.collectionName).doc(this.getDocId);
    }

    await doc.update(this.meta.fieldMapping);
  }
